  ...
  <div class="factory">
    <div class="table-region">
      ...
      <div class="search-region">
        <vxe-toolbar ref="Toolbar" :refresh="{query: getFactoryList}" custom class="toolbar">
          <template #buttons>
      ...
      <div class="table-content">
        <vxe-table ref="table" class="table myTable-scrollbar " :data="factoryList" keep-source stripe resizable show-overflow border :loading="loading"
          ...
          <vxe-table-column field="id" width="90" title="ID"/>
          <vxe-table-column field="name" title="厂家名"/>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" top="5vh" width="900px" @close="cancelFrom">
      <el-form :model="activeForm" label-width="80px" :rules="rules" ref="activeForm" size="mini">
        <el-row>
          <el-col :span="24">
            <el-form-item label="厂家名" prop="name">
              <el-input v-model="activeForm.name" placeholder="请输入厂家名"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="联系人" prop="contact">
              <el-input v-model="activeForm.contact" placeholder="请输入联系人"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="联系电话" prop="mobile">
              <el-input v-model="activeForm.mobile" placeholder="请输入联系电话"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="地址" prop="address">
              <el-input v-model="activeForm.address" type="textarea" autosize placeholder="请输入地址"/>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    addClick() {
      this.showFromEvent('新增厂家', {},  'add')
    },
    editClick() {
      let ids = this.getCheckboxRecordsEvent('修改', '厂家')
      if (ids.length > 0) this.showFromEvent('编辑' + ids.length + '个厂家资料', {ids:ids},  'multiUpdate')
    },
    editRowClick(row) {
      ...
      fastPost(baseUrl + this.activeAction, this.activeForm,true).then(res => {
          ...
          switch (this.activeAction) {
            case 'add':
              ...
              break
            case 'update':
              this.factoryList = this.factoryList.map(item => item.id === this.activeForm.id ? this.activeForm : item)
              break
            case 'multiUpdate':
    ...
    multiUpdateEvent(){
      this.activeForm.ids.map(id => this.factoryList.map(item => {
          if (item.id === id) {
    ...
    deleteClick() {
      let ids = this.getCheckboxRecordsEvent('删除', '厂家')
      if (ids.length > 0) this.showConfirmForm('正在删除' + ids.length + '个厂家', ids)
    },
    deleteRowClick(row) {
    ...
    exportSelect: myDebounce(function () {
      if (this.$refs.table.getCheckboxRecords(1).length === 0) return this.$notify.error({ title: '提示',  message: '请选择要导出的厂家',  position: 'bottom-left' })
      this.exportEvent('厂家列表(选中导出)【'+formatDate(new Date(),'yyyy-MM-dd hh:mm:ss')+'】',this.$refs.table.getCheckboxRecords(1),this.exportColumns)
    }),
